...
# Obter nome deste script para excluí-lo da listagem
SCRIPT_NAME=$(basename "$0")
echo "Este script ($SCRIPT_NAME) será ignorado durante o processamento"

# Definir nome do arquivo de saída
OUTPUT_FILE="repositorio_completo.md"
TREE_FILE="tree.md"

# Exibir arquivos que serão ignorados
echo "Arquivos que serão ignorados:"
echo "- $SCRIPT_NAME (este script)"
echo "- $OUTPUT_FILE (arquivo de saída)"
echo "- $TREE_FILE (arquivo de estrutura)"
...
should_ignore_file() {
    ...
    # Ignorar os arquivos de saída que este script cria
    if [ "$base_name" = "$OUTPUT_FILE" ] || [ "$base_name" = "$TREE_FILE" ]; then
        return 0  # Deve ignorar
    fi
    
    # Não ignorar outros arquivos
    return 1
}
...
while IFS= read -r FILE_PATH; do
    ...
    if should_ignore_file "$FILE_PATH"; then
        echo "Ignorando: $(basename "$FILE_PATH")"
        continue
    fi
...
echo "Total de arquivos processados: $PROCESSED_COUNT"
echo "Todos os arquivos foram combinados em $OUTPUT_FILE"
if [ -f "$TREE_FILE" ]; then
    echo "A estrutura de diretórios foi salva em $TREE_FILE"
fi

# Adicionar um resumo ao final do arquivo de saída
echo "# Resumo do processamento" >> "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
echo "* Data de execução: $(date)" >> "$OUTPUT_FILE"
echo "* Total de arquivos processados: $PROCESSED_COUNT" >> "$OUTPUT_FILE"
echo "* Script utilizado: $SCRIPT_NAME" >> "$OUTPUT_FILE"
